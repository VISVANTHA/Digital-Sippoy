...
- Set `NEXTAUTH_SECRET` in any real deployment — a documented, insecure-by-design default is used
  ...
  in this repo.
- `lib/require-session.ts` checks the session via `next-auth/jwt`'s `getToken()`, which accepts a
  Pages Router `NextApiRequest` directly (it's `IncomingMessage`-based) — the same reason App
  Router branches use `getToken()` over `getServerSession()` there: it keeps the handler directly
  callable from tests without depending on Next's implicit request context.
...
- **Duplication** — `npm run dup` (jscpd, config loaded explicitly via `--config jscpd.json`).
  ...
  it's paired with.
- **Tests** — `npm test` (mocha + ts-node; API route tests use `node-mocks-http` for realistic
  `NextApiRequest`/`NextApiResponse` doubles, since Pages Router routes aren't Web `Request`/
  `Response`-based like App Router's) / `npm run test:coverage` (nyc,
...
Builds successfully with `pnpm` + Webpack on Node.js 20.x. Docker/`docker-compose.yml` follow
